...
const DISSONANCE_METRO_CACHE_PATH = "caches/metro_modules.json";
...
export async function initMetroCache() {
    if (!await fileExists(DISSONANCE_METRO_CACHE_PATH)) return void buildInitCache();
    const rawCache = await readFile(DISSONANCE_METRO_CACHE_PATH);
