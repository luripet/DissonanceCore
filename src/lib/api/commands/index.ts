import { ApplicationCommand, ApplicationCommandInputType, ApplicationCommandType, DissonanceApplicationCommand } from "@lib/api/commands/types";
...
export function patchCommands() {
    const unpatch = after("getBuiltInCommands", commandsModule, ([type], res: ApplicationCommand[]) => {
        return [...res, ...commands.filter(c =>
            ...
            && c.__dissonance?.shouldHide?.() !== false)
        ];
    });
...
export function registerCommand(command: DissonanceApplicationCommand): () => void {
