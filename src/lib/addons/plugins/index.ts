...
import { createDissonancePluginApi } from "./api";
import * as t from "./types";

type PluginInstantiator = (
    dissonance: t.DissonancePluginObject,
...
export const registeredPlugins = new Map<string, t.DissonancePluginManifest>();
export const pluginInstances = new Map<string, t.PluginInstanceInternal>();
export const apiObjects = new Map<string, ReturnType<typeof createDissonancePluginApi>>();
...
function isExternalPlugin(manifest: t.DissonancePluginManifest): manifest is t.DissonancePluginManifestInternal {
...
export async function updateAndWritePlugin(repoUrl: string, id: string, fetchScript: boolean) {
    const manifest: t.DissonancePluginManifestInternal = await fetchJSON(repoUrl, `builds/${id}/manifest.json`);
...
export async function updateRepository(repoUrl: string) {
    ...
    for (const id of pluginIds) {
        const manifest = getPreloadedStorage<t.DissonancePluginManifest>(`plugins/manifests/${id}.json`);
    ...
    if (isExternalPlugin(manifest)) {
        ...
        try {
            ...
            var instantiator = globalEvalWithSourceUrl(
                `(dissonance,definePlugin)=>{${iife};return plugin?.default ?? plugin;}`,
                `dissonance-plugin/${id}-${manifest.version}`
        ...
        try {
            const api = createDissonancePluginApi(id);
